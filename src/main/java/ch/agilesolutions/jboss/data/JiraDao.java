...
//		}
	...
	public void changeStatus(String key, String comment) {
		...
		//
		// final Iterable<Transition> transitions = issueRestClient.getTransitions(issue, pm);
		//
		// final Transition resolveIssueTransition = getTransitionByName(transitions);
		//
		// if (resolveIssueTransition != null) {
		// final TransitionInput transitionInput = new TransitionInput(resolveIssueTransition.getId(), Comment.valueOf(comment));
		//
		//
		// issueRestClient.transition(issue.getTransitionsUri(),transitionInput , pm);
		// }
		//
		// } catch (Exception e) {
		// logger.warn("UC2-1 error submitting comment text to JIRA", e);
		// throw new IllegalStateException("JiraDao:changeStatus : error while change status on JIRA ticket", e);
		// }
...
//		try {
//
//			SearchResult result = client.getSearchClient().searchJql(String.format("project = %s and status in ('In Progress') and issuetype = task", projectKey), pm);
//
...
//		}
	...
	public List<Issue> checkClosedTickets() {
		...
		SearchResult result = client.getSearchClient().searchJql("project = JPE and status in (Closed,Rejected) and updatedDate > -4h", pm);
	...
	public BasicIssue saveIssue(String msg) throws Exception {
		...
		try {
			...
			// populate issue fields
			com.atlassian.jira.rest.client.domain.input.IssueInputBuilder issueBuilder = new IssueInputBuilder("TEST", new Long(27));
			...
			issueBuilder.setFieldInput(new FieldInput(IssueFieldId.TIMETRACKING_FIELD, new ComplexIssueInputFieldValue(map)));
